# AI-Based Airway Obstruction Detection

## Overview

AI-Based Airway Obstruction Detection is a machine learning-based respiratory screening platform developed to analyze breathing sound recordings for the early detection of airway obstruction. The application combines digital signal processing, machine learning, FastAPI, Streamlit, and SQLite to provide real-time prediction, patient history management, visualization, and automated PDF report generation.

The platform is designed as an intelligent screening tool to assist healthcare professionals by providing rapid respiratory assessment based on acoustic analysis.

> **Disclaimer:** This application is intended for educational and research purposes only and should not be considered a substitute for professional medical diagnosis.

---

## Features

- Machine learning-based airway obstruction prediction
- Breathing sound analysis using Librosa
- FastAPI backend for prediction services
- Interactive Streamlit web application
- Patient information management
- SQLite database for storing patient history
- Automatic PDF diagnostic report generation
- Confidence score visualization
- Historical prediction analysis
- Modular and scalable architecture

---

## System Architecture

```
                    Streamlit Frontend
                           │
                           ▼
                    FastAPI Backend
                           │
        ┌──────────────────┴──────────────────┐
        │                                     │
        ▼                                     ▼
 Machine Learning Model              SQLite Database
        │                                     │
        └──────────────► PDF Report ◄─────────┘
```

---

## Technologies Used

- Python
- Streamlit
- FastAPI
- TensorFlow / Keras
- Librosa
- NumPy
- Pandas
- Scikit-learn
- SQLite
- ReportLab
- Matplotlib

---

## Project Structure

```
AI-Airway-Obstruction-Detection/
│
├── labels/
├── models/
├── src/
│   ├── api.py
│   ├── streamlit_app.py
│   ├── predict.py
│   ├── train_cnn.py
│   ├── extract_features.py
│   ├── create_labels.py
│   ├── read_audio.py
│   ├── read_annotation.py
│   └── ...
│
├── test_audio/
├── requirements.txt
├── .gitignore
└── README.md
```

---

## Installation

Clone the repository.

```bash
git clone https://github.com/rvitmonisha/AI-Airway-Obstruction-Detection.git
```

Navigate to the project directory.

```bash
cd AI-Airway-Obstruction-Detection
```

Install the required dependencies.

```bash
pip install -r requirements.txt
```

---

## Running the Application

### Start the FastAPI Backend

```bash
python src/api.py
```

### Launch the Streamlit Application

```bash
streamlit run src/streamlit_app.py
```

After launching the application, open the local URL displayed in the terminal (typically `http://localhost:8501`) in your web browser.

---

## Workflow

1. Enter patient information.
2. Upload a breathing sound (.wav) file.
3. Audio features are extracted using Librosa.
4. The trained machine learning model predicts airway obstruction.
5. Prediction confidence is refined using the Bio-Inspired Adaptive Airway Intelligence System (BAAIS).
6. Results are stored in the SQLite database.
7. A downloadable PDF diagnostic report is generated.
8. Historical patient records are displayed in the dashboard.

---

## Results

The application provides:

- Airway obstruction prediction from breathing sound recordings.
- Prediction confidence score.
- Interactive patient history visualization.
- Automatic PDF diagnostic report generation.
- Real-time prediction using FastAPI and Streamlit.

---

## Future Enhancements

- Cloud deployment
- Multi-class respiratory disease detection
- Enhanced deep learning models
- Mobile application support
- Doctor authentication and patient management
- Hospital database integration
- Real-time respiratory monitoring

---

## Authors

| Name | Role | GitHub |
|------|------|--------|
| **M N Monisha** | Frontend Development, Database Integration, PDF Report Generation, System Integration | https://github.com/rvitmonisha |
| **Gagan U H** | Machine Learning, Signal Processing, FastAPI Backend Development | https://github.com/GaganUH |

---

## Disclaimer

This project has been developed for academic and research purposes. The predictions generated by this system are intended only as a preliminary screening aid and should not be considered a replacement for professional medical diagnosis, clinical evaluation, or standard pulmonary function testing.
